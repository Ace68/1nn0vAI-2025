...
public sealed class RinnovaBraceHandler : ICommandHandlerAsync<RinnovaBrace>, IDisposable
{
    private readonly IRepository _repository;

    public RinnovaBraceHandler(IRepository repository)
    {
        _repository = repository ?? throw new ArgumentNullException(nameof(repository));
    }

    public async Task HandleAsync(RinnovaBrace command, CancellationToken cancellationToken = default)
    {
        ...
        spiedo.RinnovaBrace(command.DataOperazione);
